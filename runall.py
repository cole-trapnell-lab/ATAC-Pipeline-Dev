...
if __name__ == '__main__':
	...
	parser.add_argument('-G','--genome', help = 'Path to genome annotation (default = /net/trapnell/vol1/genomes/human/hg19/bowtie2/)', default='/net/trapnell/vol1/genomes/human/hg19/bowtie2/hg19', dest='genome')
	parser.add_argument('-p','--nthreads', help='Number of cores available (default = 1)', default=1, dest='nthreads')
	...
	parser.add_argument('--force_overwrite_mapping', action='store_true', help='Force overwrite mapping regardless of files already present.')
	...
	if not os.path.exists(OUTPUT_PREFIX + ".split.bam") or args.force_overwrite_all or args.force_overwrite_mapping:
		print 'bowtie2 -X 1000 -p %s -x %s -1 %s -2 %s | samtools view -Sb - > %s .split.bam' % (args.nthreads, args.genome, trimmer_out1, trimmer_out2, OUTPUT_PREFIX)
